...
def run_file(filename):
    ...
    def minimum_list():
        ...
        for i in range(len(solution) - 2):
            for k in range(i, len(solution) - 2):
                city1 = solution[i + 1]
                city2 = solution[k + 2]

                new_solution = reverse(city1, city2, solution)
                cost = total_cost_func(new_solution)

                solcost_list.append((new_solution, cost))

        return solcost_list
    
    global dict_best_cost_found
    while True:
        minimum_list_result = minimum_list()
        costs = [i[1] for i in minimum_list_result]
        min_cost = min(costs)
        index = costs.index(min_cost)
        ls_solution = minimum_list_result[index][0]
        while_end = time.time()
        while_time = while_end - start3
        seconds_to_run = 1800
        if (min_cost < cost):
            print('A better solution was found based on local search reverse method:')
            print(ls_solution)
            print('cost:', min_cost, '\n')
            solution = ls_solution
            cost = min_cost
            dict_best_cost_found = cost
            if while_time > seconds_to_run:
                print('Time limit of {} seconds was reached'.format(seconds_to_run))
                decrease = cost - best_found_cost
                gap = round(abs(decrease / cost * 100), 4)

                print('improved cost by', best_found_cost -
...
with open('csvreport.csv', mode='w+') as report_csv:
    ...
    report_writer.writerow(['filename', 'best_cost_found', 'local_search_time', 'localsearch_cost', 'gap'])
    ...
    for i in range(len(all_dictionary)):
        ...
        report_writer.writerow([name, best_found_cost, local_search_time, localsearch_cost, gap])
